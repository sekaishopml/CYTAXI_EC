...
export function FormState({ destQuery, setDestQuery, destSuggestions, dest, onSearch, onSelect, onConfirm, loading, pickupAddress }: FormStateProps) {
  return (
    <div style={{ flex: 1, display: "flex", flexDirection: "column", padding: "4px 14px 12px", justifyContent: "space-between" }}>
      <p style={{ fontSize: 15, fontWeight: 600, color: T1, margin: 0 }}>
        Buenos días, <span style={{ color: G }}>Alex</span>
      </p>
      <div style={{ background: "rgba(255,255,255,0.92)", backdropFilter: "blur(16px)", borderRadius: 14, boxShadow: "0px 4px 20px rgba(0,0,0,0.05)", border: "1px solid #bbcabf33", padding: 10, display: "flex", flexDirection: "column", gap: 8, position: "relative", overflow: "hidden" }}>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, background: INPUT, borderRadius: 10, padding: "8px 10px", border: "1px solid #006c494D" }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <input style={{ background: "transparent", border: "none", padding: 0, fontSize: 14, fontFamily: "Inter", color: T1, width: "100%", outline: "none" }}
              placeholder="Buscar destino" value={destQuery} onChange={e => { setDestQuery(e.target.value); onSearch(e.target.value); }} autoFocus />
          </div>
        </div>
      ...
      <button onClick={onConfirm} disabled={!dest || loading}
        style={{ width: "100%", height: 42, background: "#000", color: "#fff", borderRadius: 9999, fontSize: 15, fontWeight: 600, fontFamily: "Inter", border: "none", cursor: "pointer", boxShadow: "0px 4px 12px rgba(0,0,0,0.2)", opacity: (!dest || loading) ? 0.4 : 1, transition: "opacity 0.2s" }}>
        {loading ? "Calculando..." : "Buscar viaje"}
      </button>
    </div>
  );
}
